...
echo "PHYSICAL E2E: collecting serial boot evidence"
(
  timeout --signal=TERM 75s pio device monitor --port "$PORT" --baud 115200 || true
) >"$LOG" 2>&1 &
MONITOR_PID=$!
...
echo "PHYSICAL E2E: probing device HTTP health at $HEALTH_URL"
...
for key, value in expected.items():
    if payload.get(key) != value:
        raise SystemExit(f"PHYSICAL E2E: FAIL /health field {key!r} expected {value!r}, got {payload.get(key)!r}")
print("PHYSICAL E2E: device /health PASS (ok=true, board=ESP32-4848S040, wifi=true, backend=true)")
PY

cp "$LOG" "${GITHUB_WORKSPACE:-.}/esp32-physical-e2e.log"
echo "PHYSICAL E2E: PASS (USB flash + boot/display init + Wi-Fi + backend health + device HTTP health)"
echo "PHYSICAL E2E: optical/touch behavior still requires direct human or camera-based evidence"
